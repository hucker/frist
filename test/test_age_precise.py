...
@pytest.mark.parametrize(
    ...
    [
        # Same year, not leap
        (dt.datetime(2021, 1, 1), dt.datetime(2021, 7, 2), (dt.datetime(2021, 7, 2) - dt.datetime(2021, 1, 1)).days / 365),
        # Leap year, same year
        (dt.datetime(2020, 2, 1), dt.datetime(2020, 8, 1), (dt.datetime(2020, 8, 1) - dt.datetime(2020, 2, 1)).days / 366),
        # Across leap year
        (dt.datetime(2019, 7, 1), dt.datetime(2020, 7, 1), ((dt.datetime(2019, 12, 31, 23, 59, 59) - dt.datetime(2019, 7, 1)).days / 365) + ((dt.datetime(2020, 7, 1) - dt.datetime(2020, 1, 1)).days / 366)),
...
def test_months_property_normalized_lengths_all_months() -> None:
    ...
    for year in range(2000, 2006):
        for month in range(1, 13):
            ...
            end = dt.datetime(year if month < 12 else year + 1, month + 1 if month < 12 else 1, 1)
            age = Age(start, end)
            normalized = age.months
            actuals.add(normalized)

  
    expected:set[float] = {28/DAYS_PER_MONTH, 29/DAYS_PER_MONTH, 30/DAYS_PER_MONTH, 31/DAYS_PER_MONTH}
    
    
    # This equality should work even with sets of floats since the rations are exactly the same
    assert expected == actuals
    assert DAYS_PER_MONTH == 30.44   # Sanity check but if you change the constant by mistake this will catch it
...
def test_months_precise_same_month(same_month_start: dt.datetime, same_month_end: dt.datetime) -> None:
    """
    Test Age.months_precise for intervals where start and end are in the same month, including time portions and edge cases.
    """
    # Arrange
    age = Age(same_month_start, same_month_end)
    # Act
    actual = age.months_precise
    # Compute expected value using the same logic as months_precise
    month_start = dt.datetime(same_month_start.year, same_month_start.month, 1)
...
def test_months_precise_start_equals_end() -> None:
    ...
    """
    start = dt.datetime(2023, 5, 1, 12, 0)
    end = dt.datetime(2023, 5, 1, 12, 0)
    age = Age(start, end)
    actual = age.months_precise
    assert actual == 0.0
...
def test_months_precise_february_non_leap(end_day: int, expected: float) -> None:
    ...
    end = dt.datetime(2023, 2, end_day)
    age = Age(start, end)
    actual = pytest.approx(age.months_precise, 0.01)
    assert actual == expected