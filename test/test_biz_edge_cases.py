...
def test_workday_fraction_zero_length_business_day() -> None:
    ...
    # Act/Assert
    actual = biz._workday_fraction_at(target)
    expected = 0.0
...
def test_workday_fraction_before_and_after() -> None:
    ...
    # Act/Assert
    actual_before = biz_before._workday_fraction_at(biz_before.target_dt)
    expected_before = 0.0
    assert actual_before == expected_before, (
        f"Workday fraction (before) mismatch: expected={expected_before}, actual={actual_before}"
    )

    actual_after = biz_after._workday_fraction_at(biz_after.target_dt)
    expected_after = 1.0
...
def test_move_n_days_zero_returns_same_date() -> None:
    ...
    # Act/Assert
    actual_business = biz._move_n_days(d, 0, count_business=True)
    actual_calendar = biz._move_n_days(d, 0, count_business=False)
    expected = d
